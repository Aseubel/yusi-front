...
interface EncryptionState {
    ...
    switchToCustomMode: (password: string, enableCloudBackup?: boolean) => Promise<void>;
    changeCustomPassword: (oldPassword: string, newPassword: string, enableCloudBackup?: boolean) => Promise<void>;
        ...
        (set, get) => ({
            ...
            initialize: async () => {
                ...
                try {
                    ...
                    // 如果是 DEFAULT 模式，自动导入服务端密钥
                    if (settings.keyMode === 'DEFAULT' && settings.serverKey) {
                        const key = await importKeyFromBase64(settings.serverKey);
                        set({ cryptoKey: key });
                    }
                    // 如果是 CUSTOM 模式且有保存的密码，自动派生密钥
                    else if (settings.keyMode === 'CUSTOM' && settings.keySalt) {
            ...
            switchToDefaultMode: async () => {
                ...
                try {
                    ...
                    // 2. 获取新的 DEFAULT 模式设置（会生成服务端密钥）
                    const request: KeyModeUpdateRequest = { keyMode: 'DEFAULT' };
                    await apiUpdateKeyMode(request);

                    // 3. 重新获取设置以获取服务端密钥
                    const newSettings = await fetchKeySettings();
                    if (!newSettings.serverKey) {
                        throw new Error('服务端密钥获取失败');
                    }

                    // 4. 导入新密钥
                    const newKey = await importKeyFromBase64(newSettings.serverKey);
            ...
            switchToCustomMode: async (password: string, enableCloudBackup = false) => {
                set({ isLoading: true, error: null });
                try {
                    // 1. 生成新盐值
                    ...
                    if (enableCloudBackup) {
                        // TODO: 使用管理员公钥加密密码
                        // 暂时使用简单的 base64 编码作为占位
                        encryptedBackupKey = btoa(password);
                    }
            ...
            changeCustomPassword: async (oldPassword: string, newPassword: string, enableCloudBackup = false) => {
                const { keySalt } = get();
                if (!keySalt) {
                ...
                try {
                    ...
                    let encryptedBackupKey: string | undefined;
                    if (enableCloudBackup) {
                        encryptedBackupKey = btoa(newPassword);
                    }
