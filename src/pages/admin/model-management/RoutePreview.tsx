        ...
        <>
          ...
          <div className="space-y-2">
            {preview.candidates.map((candidate, index) => {
              ...
              return <div key={`${candidate.tierId}-${candidate.modelId}-${index}`} className={`grid gap-3 border p-3 sm:grid-cols-[minmax(110px,0.7fr)_minmax(140px,1fr)_minmax(160px,1.3fr)] sm:items-center ${unavailable ? 'border-rose-200 bg-rose-50/50 dark:border-rose-900/50 dark:bg-rose-950/20' : 'border-border bg-background'}`}>
                <div className="flex items-center justify-between gap-2 text-xs font-semibold sm:justify-start"><span className="font-mono text-muted-foreground">{index + 1}</span><span className="font-mono">{candidate.tierId}</span></div>
                <div className="flex min-w-0 items-center justify-between gap-2 sm:justify-start"><span className="flex min-w-0 items-center gap-2"><Server className="h-4 w-4 shrink-0 text-sky-600" aria-hidden="true" /><span className="truncate font-mono text-sm">{candidate.modelId}</span></span><span className="text-[11px] uppercase tracking-[0.1em] text-muted-foreground sm:hidden">{t('modelManagement.registry.model')}</span></div>
                <div className="flex flex-wrap items-center gap-2 text-xs text-muted-foreground"><span className="text-[11px] uppercase tracking-[0.1em] sm:hidden">{t('modelManagement.registry.provider')}</span><span>{candidate.provider || t('modelManagement.registry.unsupportedProvider')}</span>{candidate.modelName && <span className="truncate">{candidate.modelName}</span>}{unavailable ? <span className="inline-flex items-center gap-1 text-rose-600 dark:text-rose-300"><CircleX className="h-3.5 w-3.5" aria-hidden="true" />{candidate.excludedReason}</span> : candidate.excludedReason === 'fallback-tier' ? <Badge variant="outline">{t('modelManagement.preview.fallbackCandidate')}</Badge> : <span className="inline-flex items-center gap-1 text-emerald-600 dark:text-emerald-300"><CheckCircle2 className="h-3.5 w-3.5" aria-hidden="true" />{t('modelManagement.preview.available')}</span>}</div>
              </div>
            })}
          </div>
