...
export const ModelCallActivity = ({ tiers, providers }: ModelCallActivityProps) => {
  ...
  return (
    <section className="space-y-4" aria-labelledby="model-activity-heading">
      <div className="flex flex-col gap-3 md:flex-row md:items-end md:justify-between"><div><h2 id="model-activity-heading" className="flex items-center gap-2 text-lg font-semibold"><Activity className="h-5 w-5 text-primary" aria-hidden="true" />{t('modelManagement.activity.title')}</h2><p className="mt-1 text-sm text-muted-foreground">{t('modelManagement.activity.subtitle')}</p></div><Button variant="outline" size="icon" className="h-11 w-11 sm:h-9 sm:w-9" onClick={() => void load()} disabled={loading} aria-label={t('common.refresh')} title={t('common.refresh')}><RotateCcw className={`h-4 w-4 ${loading ? 'animate-spin' : ''}`} aria-hidden="true" /></Button></div>
      <div className="flex flex-col gap-3 border-y border-border py-3 sm:grid sm:grid-cols-2 sm:gap-2 lg:grid-cols-4"><Input className="min-h-11 sm:min-h-10" value={filters.scene ?? ''} onChange={(event) => updateFilter('scene', event.target.value || undefined)} placeholder={t('modelManagement.activity.sceneFilter')} /><Select value={filters.modelTier ?? 'ALL'} onValueChange={(value) => updateFilter('modelTier', value === 'ALL' ? undefined : value)} options={[{ value: 'ALL', label: t('modelManagement.health.allTiers') }, ...tiers.map((tier) => ({ value: tier, label: tier }))]} /><Select value={filters.provider ?? 'ALL'} onValueChange={(value) => updateFilter('provider', value === 'ALL' ? undefined : value)} options={[{ value: 'ALL', label: t('modelManagement.health.allProviders') }, ...providers.map((provider) => ({ value: provider, label: provider }))]} /><Select value={filters.fallbackUsed == null ? 'ALL' : String(filters.fallbackUsed)} onValueChange={(value) => updateFilter('fallbackUsed', value === 'ALL' ? undefined : value)} options={[{ value: 'ALL', label: t('modelManagement.activity.allAttempts') }, { value: 'true', label: t('modelManagement.activity.fallbackOnly') }, { value: 'false', label: t('modelManagement.activity.primaryOnly') }]} /><Input className="min-h-11 sm:min-h-10" type="datetime-local" value={filters.from ?? ''} onChange={(event) => updateFilter('from', event.target.value || undefined)} aria-label={t('modelManagement.activity.from')} /><Input className="min-h-11 sm:min-h-10" type="datetime-local" value={filters.to ?? ''} onChange={(event) => updateFilter('to', event.target.value || undefined)} aria-label={t('modelManagement.activity.to')} /></div>

      <div className="overflow-hidden border border-border bg-background">
        <div className="hidden grid-cols-[150px_minmax(110px,0.8fr)_minmax(145px,1fr)_100px_100px_110px_110px] gap-3 border-b border-border bg-muted/30 px-4 py-3 text-[11px] font-semibold uppercase tracking-[0.12em] text-muted-foreground lg:grid"><span>{t('modelManagement.activity.time')}</span><span>{t('modelManagement.activity.scene')}</span><span>{t('modelManagement.activity.model')}</span><span>{t('modelManagement.activity.tier')}</span><span>{t('modelManagement.activity.status')}</span><span>{t('modelManagement.activity.latency')}</span><span>{t('modelManagement.activity.usage')}</span></div>
        {loading ? <div className="flex min-h-40 items-center justify-center"><Loader2 className="h-6 w-6 animate-spin text-primary" aria-label={t('modelManagement.activity.loading')} /></div> : items.map((item) => <Button key={item.attemptId} type="button" variant="ghost" onClick={() => setSelected(item)} className="h-auto min-h-[72px] w-full flex-col items-stretch justify-start gap-3 rounded-none px-4 py-4 text-left active:scale-100 hover:bg-muted/30 lg:grid lg:grid-cols-[150px_minmax(110px,0.8fr)_minmax(145px,1fr)_100px_100px_110px_110px] lg:items-center lg:gap-3">
          <ActivityMetric label={t('modelManagement.activity.time')} value={formatDate(item.createdAt)} />
          <ActivityMetric label={t('modelManagement.activity.scene')} value={item.scene} emphasis />
          <span className="flex min-w-0 items-start justify-between gap-3 lg:block"><span className="text-[11px] uppercase tracking-[0.1em] text-muted-foreground lg:hidden">{t('modelManagement.activity.model')}</span><span className="min-w-0 text-right lg:block lg:text-left"><span className="block truncate font-mono text-sm">{item.modelId || '-'}</span><span className="block truncate text-xs text-muted-foreground">{item.provider || '-'}</span></span></span>
          <ActivityMetric label={t('modelManagement.activity.tier')} value={item.selectedTier || item.primaryTier || '-'} mono />
          <div className="flex items-center justify-between gap-3 lg:block"><span className="text-[11px] uppercase tracking-[0.1em] text-muted-foreground lg:hidden">{t('modelManagement.activity.status')}</span><Badge variant={item.status === 'SUCCESS' || item.status === 'COMPLETED' ? 'secondary' : 'destructive'}>{item.fallbackUsed ? t('modelManagement.activity.fallback') : item.status}</Badge></div>
          <div className="flex items-center justify-between gap-3 text-sm tabular-nums lg:justify-start"><span className="text-[11px] uppercase tracking-[0.1em] text-muted-foreground lg:hidden">{t('modelManagement.activity.latency')}</span><span className="flex items-center gap-1"><Clock3 className="h-3.5 w-3.5 text-muted-foreground" aria-hidden="true" />{item.latencyMs == null ? '-' : `${item.latencyMs} ms`}</span></div>
          <ActivityMetric label={t('modelManagement.activity.usage')} value={`${item.inputTokens ?? 0} → ${item.outputTokens ?? 0}`} mono />
        </Button>)}
        {!loading && !items.length && <div className="flex min-h-40 items-center justify-center gap-2 text-sm text-muted-foreground"><Filter className="h-4 w-4" aria-hidden="true" />{t('modelManagement.activity.empty')}</div>}
      </div>

      {totalElements > 0 && <div className="flex items-center justify-center gap-3"><Button variant="outline" size="icon" className="h-11 w-11 sm:h-9 sm:w-9" onClick={() => updateFilter('page', Math.max(0, filters.page - 1))} disabled={filters.page === 0 || loading} aria-label={t('modelManagement.activity.previous')} title={t('modelManagement.activity.previous')}><ArrowLeft className="h-4 w-4" aria-hidden="true" /></Button><span className="text-sm tabular-nums text-muted-foreground">{currentPage} / {pageCount}</span><Button variant="outline" size="icon" className="h-11 w-11 sm:h-9 sm:w-9" onClick={() => updateFilter('page', Math.min(pageCount - 1, filters.page + 1))} disabled={filters.page >= pageCount - 1 || loading} aria-label={t('modelManagement.activity.next')} title={t('modelManagement.activity.next')}><ArrowRight className="h-4 w-4" aria-hidden="true" /></Button></div>}
...
const ActivityMetric = ({ label, value, mono = false, emphasis = false }: { label: string; value: string; mono?: boolean; emphasis?: boolean }) => (
  <span className="flex min-w-0 items-start justify-between gap-3 lg:block">
    ...
    <span className={`${mono ? 'font-mono text-xs' : emphasis ? 'font-medium' : 'text-muted-foreground'} min-w-0 truncate text-right lg:text-left`}>{value}</span>
  </span>
)
