...
export const ModelManagement = () => {
  ...
  return (
    <div className="space-y-5 pb-8 sm:space-y-6">
      ...
      {error && <div className="flex items-start gap-2 border border-rose-200 bg-rose-50 px-3 py-3 text-sm text-rose-700 dark:border-rose-900/50 dark:bg-rose-950/20 dark:text-rose-200 sm:px-4"><AlertTriangle className="mt-0.5 h-4 w-4 shrink-0" aria-hidden="true" />{error}</div>}
      {conflict && <div className="flex flex-col gap-3 border border-amber-300 bg-amber-50 px-3 py-3 text-sm text-amber-800 dark:border-amber-700/60 dark:bg-amber-950/20 dark:text-amber-200 sm:px-4 md:flex-row md:items-center md:justify-between"><div className="flex items-start gap-2"><AlertTriangle className="mt-0.5 h-4 w-4 shrink-0" aria-hidden="true" /><span>{t('modelManagement.console.conflictDescription', { version: conflict.server.version })}</span></div><div className="flex flex-col gap-2 sm:flex-row sm:shrink-0"><Button variant="outline" size="sm" className="min-h-11 sm:min-h-9" onClick={() => { const nextDraft = createGovernanceDraft(conflict.server); setSnapshot(conflict.server); setRuntimeStates(conflict.server.runtimeStates ?? []); setMetrics(conflict.server.summary ?? emptyMetrics); setDraft(nextDraft); setConflict(null) }}>{t('modelManagement.console.reloadServer')}</Button><Button variant="ghost" size="sm" className="min-h-11 sm:min-h-9" onClick={() => setConflict(null)}>{t('modelManagement.console.keepDraft')}</Button></div></div>}
      ...
      <section className="border-t border-border pt-4" aria-label={t('modelManagement.advanced.title')}>
        ...
        {advancedOpen && <div className="mt-3 border border-border bg-muted/10 p-4"><div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><div><h3 className="flex items-center gap-2 text-sm font-semibold"><FileJson className="h-4 w-4 text-primary" aria-hidden="true" />{t('modelManagement.advanced.title')}</h3><p className="mt-1 text-xs text-muted-foreground">{t('modelManagement.advanced.description')}</p></div><div className="flex flex-col gap-2 sm:flex-row"><Button variant="outline" size="sm" onClick={() => void copyDraft()} className="min-h-11 w-full gap-2 sm:min-h-9 sm:w-auto"><Copy className="h-4 w-4" aria-hidden="true" />{t('modelManagement.advanced.copy')}</Button><Button variant="outline" size="sm" onClick={exportDraft} className="min-h-11 w-full gap-2 sm:min-h-9 sm:w-auto"><Download className="h-4 w-4" aria-hidden="true" />{t('modelManagement.advanced.export')}</Button></div></div><pre className="mt-4 max-h-80 overflow-auto border border-border bg-background p-3 text-xs leading-5 text-muted-foreground">{JSON.stringify(toUpdateRequest(draft), null, 2)}</pre></div>}
      </section>

      <div className="sticky bottom-3 z-20 flex flex-col gap-3 border border-border bg-background/95 p-3 shadow-lg backdrop-blur sm:flex-row sm:items-center sm:justify-between"><div className="flex flex-wrap items-center gap-2 text-xs text-muted-foreground"><span className={`h-2 w-2 rounded-full ${isDirty ? 'bg-amber-500' : 'bg-emerald-500'}`} />{isDirty ? t('modelManagement.console.unsaved') : t('modelManagement.console.synced')}<span className="font-mono">v{draft.version}</span>{validationErrors.length > 0 && <span className="text-rose-600 dark:text-rose-300">{t('modelManagement.console.validationCount', { count: validationErrors.length })}</span>}</div><div className="flex flex-col gap-2 sm:flex-row sm:justify-end"><Button variant="outline" onClick={() => void Promise.all([loadConsole(true), refreshRuntime()])} disabled={loading || runtimeLoading} className="min-h-11 w-full gap-2 sm:min-h-10 sm:w-auto"><RefreshCw className={`h-4 w-4 ${loading || runtimeLoading ? 'animate-spin' : ''}`} aria-hidden="true" />{t('common.refresh')}</Button><Button onClick={() => void save()} disabled={!isDirty || validationErrors.length > 0 || saving} isLoading={saving} className="min-h-11 w-full gap-2 sm:min-h-10 sm:w-auto"><Save className="h-4 w-4" aria-hidden="true" />{t('common.save')}</Button></div></div>
    </div>
  )
}
